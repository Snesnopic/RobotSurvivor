...
extension GameScene{
    ...
    func updateTiles(){
        newCenterTile()
        for x in stride(from: centerTile.x - 1536, to: centerTile.x + 1536, by: tileSize.width){
            for y in stride(from: centerTile.y - 1536, to: centerTile.y + 1536, by: tileSize.height){
