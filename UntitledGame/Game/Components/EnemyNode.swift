...
class EnemyCentipedeNode: SKSpriteNode {
    var type: EnemyCentipedeType
    
    init(type: EnemyCentipedeType, startPosition: CGPoint) {
        self.type = type
        let texture = SKTexture(imageNamed: "\(type.name)/Head/1")
